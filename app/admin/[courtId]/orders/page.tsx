...
export default function AdminOrdersPage({ params }: { params: Promise<{ courtId: string }> }) {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const result = await adminOrdersApi.list(courtId, queryParams)
      setOrders(result.orders as Order[])
      setPagination(result.pagination)
    } catch (e) {
      toast({
        title: "Error",
        description: e instanceof ApiError ? e.message : "Failed to fetch orders",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    >
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground">Monitor and manage all food court orders</p>
        </div>
